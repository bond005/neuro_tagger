...
cross-validated estimating on the labeled text corpus in the 
``brat`` format (`brat`<http://brat.nlplab.org>) is popular tool for
manual annotating of texts);
...
entity recognition and fact extraction in Russian (it is described in
the paper `FactRuEval 2016: Evaluation of Named Entity Recognition and
Fact Extraction Systems for Russian`<http://www.dialog-21.ru/media/3430/starostinaetal.pdf>).
...
from the `iPavlov project`<https://ipavlov.ai> and CRF as final
classifier allows to reach best result for first track of this
competition (F1-score is greater than 0,89, and it is state-fo-the-art
solution for named entity recognition in Russian).
...
on corpus of long texts, then the training can be converged slowly. If you
use the neuro_tagger, trained on short texts, for recognizing of long text,
then only some initial words of this text can be tagged, and remaining
words at the end of text will not be considered by algorithm. Besides, you
need to use a very large volume of RAM for processing of long texts.

For solving of above-mentioned problem you can split long texts by shorter
sentences using well-known NLP libraries such as `NLTK`
<http://www.nltk.org/api/nltk.tokenize.html?highlight=sent_tokenize#nltk.tokenize.sent_tokenize>
or `SpaCy`<https://spacy.io/api/token#is_sent_start>. Also, if you want
to correctly split long text with its tag labels, then you can use the
special function ``tokenize_all_by_sentences`` from the module
``neuro_tagger.dataset_loading``.

Acknowledgment
---------------

The work was supported by National Technology Initiative and PAO
Sberbank project ID 0000000007417F630002.
'''
